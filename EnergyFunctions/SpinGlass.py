...
class SpinGlassEnergyClass(BaseEnergyClass):
	...
	def calculate_Energy(self, H_graph, bins, node_gr_idx):
		...
		nodes = H_graph.nodes
		couplings = jax.random.randint(self.key, (H_graph.senders.shape[0], 1), 0, 2)

		total_num_nodes = jax.tree_util.tree_leaves(nodes)[0].shape[0]

		raveled_spins = jnp.reshape(spins, (bins.shape[0], 1))

		Energy_messages = couplings*(raveled_spins[H_graph.senders]) * (raveled_spins[H_graph.receivers])
